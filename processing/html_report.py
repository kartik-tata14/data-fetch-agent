...
def _generate_insights(grouped_runs, all_runs, historical_data=None):
    """Generate intelligent, actionable business insights."""
    points = []

    total_runs = len(all_runs)
    points.append({
        "text": f"Total <strong>{total_runs}</strong> job run(s) analyzed across "
                f"<strong>{len(grouped_runs)}</strong> case count configuration(s).",
        "color": "#2F5496",
    })
    ...
    for cc, runs in grouped_runs.items():
        ...
        if total_avg:
            points.append({
                "text": f"<strong>{cc}-case runs</strong> ({len(runs)} runs): "
                        f"Average total processing time is <strong>{_fmt_td(total_avg)}</strong>.",
                "color": "#006100",
            })
        if tpc_avg:
            points.append({
                "text": f"<strong>{cc}-case runs</strong>: "
                        f"Average time per case is <strong>{_fmt_td(tpc_avg)}</strong>.",
                "color": "#006100",
            })

        # Bottleneck: which Web Job takes the most time
        max_stage = None
        max_secs = 0
        stage_pcts = {}
        for s in WJ_PROCESSING_STAGES:
            if s in avgs and total_avg and total_avg.total_seconds() > 0:
                secs = avgs[s].total_seconds()
                pct = (secs / total_avg.total_seconds()) * 100
                stage_pcts[s] = pct
                if secs > max_secs:
                    max_secs = secs
                    max_stage = s
        if max_stage:
            points.append({
                "text": f"<strong>{cc}-case runs</strong>: "
                        f"'<strong>{max_stage}</strong>' consumes <strong>{stage_pcts[max_stage]:.1f}%</strong> "
                        f"of total processing time (avg <strong>{_fmt_td(avgs[max_stage])}</strong>). "
                        f"This is the primary optimization target.",
                "color": "#C00000",
            })
        ...
        if total_avg and total_avg.total_seconds() > 0:
            ...
            severity = "Critical" if wait_pct > 30 else "Moderate" if wait_pct > 15 else "Acceptable"
            points.append({
                "text": f"<strong>{cc}-case runs</strong>: Wait time overhead is "
                        f"<strong>{wait_pct:.1f}%</strong> ({severity}). "
                        f"Total idle time <strong>{_fmt_td(total_wait)}</strong> "
                        f"out of <strong>{_fmt_td(total_avg)}</strong> total.",
                "color": color,
            })

        # Consistency analysis per stage
        if len(runs) >= 3:
            for stage in WJ_PROCESSING_STAGES:
                ...
                if stats["cv"] > 50:
                    points.append({
                        "text": f"<strong>Performance Instability</strong> in {cc}-case '{stage}': "
                                f"CV={stats['cv']:.1f}%, spread from {_secs_to_hms(stats['min'])} to "
                                f"{_secs_to_hms(stats['max'])}. "
                                f"Investigate resource contention or environment inconsistency.",
                        "color": "#C00000",
                    })
                elif stats["cv"] > 25:
                    points.append({
                        "text": f"<strong>Moderate Variability</strong> in {cc}-case '{stage}': "
                                f"CV={stats['cv']:.1f}%, range {_secs_to_hms(stats['min'])} to "
                                f"{_secs_to_hms(stats['max'])}.",
                        "color": "#ED7D31",
                    })
                elif stats["cv"] < 10:
                    points.append({
                        "text": f"<strong>Stable Performance</strong> in {cc}-case '{stage}': "
                                f"CV={stats['cv']:.1f}%, avg {_secs_to_hms(stats['avg'])}.",
                        "color": "#006100",
                    })
            ...
            for wstage in WAIT_STAGES:
                ...
                if stats["cv"] > 80:
                    points.append({
                        "text": f"<strong>Highly Unpredictable</strong> '{wstage}' in {cc}-case runs: "
                                f"CV={stats['cv']:.1f}%, range {_secs_to_hms(stats['min'])} to "
                                f"{_secs_to_hms(stats['max'])}. "
                                f"Likely caused by queue congestion or scheduling variability.",
                        "color": "#C00000",
                    })
                elif stats["cv"] > 40:
                    points.append({
                        "text": f"<strong>Variable Wait</strong> '{wstage}' in {cc}-case runs: "
                                f"CV={stats['cv']:.1f}%, avg {_secs_to_hms(stats['avg'])}.",
                        "color": "#ED7D31",
                    })

    # Historical trend insights (2000-case only)
    if historical_data and len(historical_data) >= 2:
        first = historical_data[0]
        last = historical_data[-1]
        if first["avg_total"] > 0 and last["avg_total"] > 0:
            change_pct = ((last["avg_total"] - first["avg_total"]) / first["avg_total"]) * 100
            direction = "improved" if change_pct < 0 else "regressed"
            color = "#006100" if change_pct < 0 else "#C00000"
            points.append({
                "text": f"<strong>Historical Trend (2K bulk)</strong>: Total processing time has "
                        f"<strong>{direction} by {abs(change_pct):.1f}%</strong> from {first['execution']} "
                        f"({first['date']}) to {last['execution']} ({last['date']}). "
                        f"From {_secs_to_hms(first['avg_total'])} to {_secs_to_hms(last['avg_total'])}.",
                "color": color,
            })

        # WJ2 trend (usually the bottleneck)
        if first["avg_wj2"] > 0 and last["avg_wj2"] > 0:
            wj2_change = ((last["avg_wj2"] - first["avg_wj2"]) / first["avg_wj2"]) * 100
            direction = "improved" if wj2_change < 0 else "regressed"
            color = "#006100" if wj2_change < 0 else "#C00000"
            points.append({
                "text": f"<strong>WEB JOB 2 Trend</strong>: {direction.capitalize()} by "
                        f"<strong>{abs(wj2_change):.1f}%</strong> across executions "
                        f"({_secs_to_hms(first['avg_wj2'])} -> {_secs_to_hms(last['avg_wj2'])}).",
                "color": color,
            })
...
def _render_comparison_table(runs, case_count):
    ...
    all_st = [_extract_stage_timings(r) for r in runs]
    for label in STAGE_LABELS:
        if label == "TOTAL PROCESSING TIME":
            ...
            rc = 'class="trigger-row"'
        else:
            rc = 'class="stage-row"'
        html += f'<tr {rc}><td class="label-cell">{label}</td>\n'
        for st in all_st:
            val = st.get(label, "")
            if isinstance(val, timedelta):
                val = _fmt_td(val)
...
def _render_bar_chart_svg(runs, case_count, chart_id):
    ...
    max_val = max((v for cd in chart_data for v in cd["values"]), default=1)
    if max_val == 0:
        max_val = 1
    ...
    for i in range(6):
        y = top_margin + plot_height - (i / 5) * plot_height
        val_secs = (i / 5) * max_val
        h = int(val_secs // 3600)
        m = int((val_secs % 3600) // 60)
        s = int(val_secs % 60)
        svg += f'<line x1="{left_margin}" y1="{y}" x2="{chart_width - right_margin}" y2="{y}" stroke="#e0e0e0"/>\n'
        svg += f'<text x="{left_margin - 10}" y="{y + 4}" text-anchor="end" font-size="11" fill="#666">{h:02d}:{m:02d}:{s:02d}</text>\n'

    for s_idx, stage in enumerate(stages):
        gx = left_margin + s_idx * group_width + 10
        for r_idx, cd in enumerate(chart_data):
            val = cd["values"][s_idx]
            bh = (val / max_val) * plot_height
            x = gx + r_idx * (bar_width + 4)
            y = top_margin + plot_height - bh
            color = CHART_COLORS[r_idx % len(CHART_COLORS)]
            svg += f'<rect x="{x}" y="{y}" width="{bar_width}" height="{bh}" fill="{color}" rx="2">'
            svg += f'<title>{cd["guid_short"]}...: {_secs_to_hms(val)}</title></rect>\n'
        lx = gx + (num_runs * (bar_width + 4)) / 2
        ly = top_margin + plot_height + 14
        svg += f'<text x="{lx}" y="{ly}" text-anchor="middle" font-size="12" fill="#333" font-weight="600">{stage.replace(" Processing Time", "")}</text>\n'

    svg += '</svg>\n'
    legend = '<div class="chart-legend">\n'
    for r_idx, cd in enumerate(chart_data):
        color = CHART_COLORS[r_idx % len(CHART_COLORS)]
        legend += f'<span class="legend-item"><span class="legend-box" style="background:{color}"></span>Run {r_idx+1}: {cd["guid_short"]}...</span>\n'
    legend += '</div>\n'
    return svg + legend


def _render_trend_line_svg(historical_data, metric_key, title, chart_id, color="#2F5496"):
    """Render a single-metric line chart SVG."""
    if not historical_data:
        return ""

    values = [d[metric_key] for d in historical_data]
    labels = [f"{d['execution']}\n{d['date']}" for d in historical_data]
    ...
    top_margin = 50
    bottom_margin = 80
    chart_width = max(left_margin + n * 100 + right_margin, 600)
    chart_height = 300
    plot_height = chart_height - top_margin - bottom_margin
    ...
    svg = f'<svg width="{chart_width}" height="{chart_height}" xmlns="http://www.w3.org/2000/svg" id="{chart_id}">\n'
    svg += f'<rect width="{chart_width}" height="{chart_height}" fill="#fafafa" rx="8"/>\n'
    svg += f'<text x="{chart_width/2}" y="20" text-anchor="middle" font-size="14" font-weight="600" fill="#333">{title}</text>\n'

    # Gridlines
    for i in range(6):
        y = top_margin + plot_height - (i / 5) * plot_height
        val_secs = (i / 5) * max_val
        svg += f'<line x1="{left_margin}" y1="{y}" x2="{chart_width - right_margin}" y2="{y}" stroke="#e8e8e8"/>\n'
        svg += f'<text x="{left_margin - 10}" y="{y + 4}" text-anchor="end" font-size="10" fill="#888">{_secs_to_hms(val_secs)}</text>\n'

    # Points and lines
    points = []
    for i, val in enumerate(values):
        x = left_margin + (i / max(n - 1, 1)) * plot_width if n > 1 else left_margin + plot_width / 2
        y = top_margin + plot_height - (val / max_val) * plot_height
        points.append((x, y))
    ...
    if len(points) > 1:
        ...
        for px, py in points[1:]:
            path += f' L {px},{py}'
        svg += f'<path d="{path}" fill="none" stroke="{color}" stroke-width="2.5"/>\n'

    for i, (px, py) in enumerate(points):
        svg += f'<circle cx="{px}" cy="{py}" r="5" fill="{color}" stroke="#fff" stroke-width="2">'
        svg += f'<title>{historical_data[i]["execution"]} ({historical_data[i]["date"]}): {_secs_to_hms(values[i])}</title></circle>\n'
        svg += f'<text x="{px}" y="{py - 10}" text-anchor="middle" font-size="9" fill="#555">{_secs_to_hms(values[i])}</text>\n'

    # X-axis labels
    for i, (px, _) in enumerate(points):
        ly = top_margin + plot_height + 16
        svg += f'<text x="{px}" y="{ly}" text-anchor="middle" font-size="10" fill="#555">{historical_data[i]["execution"]}</text>\n'
        svg += f'<text x="{px}" y="{ly + 13}" text-anchor="middle" font-size="9" fill="#888">{historical_data[i]["date"]}</text>\n'

    svg += '</svg>\n'
    return svg


def _render_multi_trend_svg(historical_data, chart_id):
    """Render a multi-line chart: WJ1, WJ2, WJ3 trends overlaid."""
    if not historical_data:
        return ""

    metrics = [
        ("avg_wj1", "WEB JOB 1", LINE_COLORS["avg_wj1"]),
        ("avg_wj2", "WEB JOB 2", LINE_COLORS["avg_wj2"]),
        ("avg_wj3", "WEB JOB 3", LINE_COLORS["avg_wj3"]),
    ]

    all_vals = []
    for key, _, _ in metrics:
        all_vals.extend(d[key] for d in historical_data)
    ...
    svg = f'<svg width="{chart_width}" height="{chart_height}" xmlns="http://www.w3.org/2000/svg" id="{chart_id}">\n'
    svg += f'<rect width="{chart_width}" height="{chart_height}" fill="#fafafa" rx="8"/>\n'
    svg += f'<text x="{chart_width/2}" y="20" text-anchor="middle" font-size="14" font-weight="600" fill="#333">Web Job Processing Time Trend (per 2K file)</text>\n'
    ...
    for key, label, color in metrics:
        values = [d[key] for d in historical_data]
        points = []
        for i, val in enumerate(values):
            x = left_margin + (i / max(n - 1, 1)) * plot_width if n > 1 else left_margin + plot_width / 2
            y = top_margin + plot_height - (val / max_val) * plot_height
        ...
        for i, (px, py) in enumerate(points):
            svg += f'<circle cx="{px}" cy="{py}" r="4" fill="{color}" stroke="#fff" stroke-width="1.5">'
            svg += f'<title>{label} - {historical_data[i]["execution"]}: {_secs_to_hms(values[i])}</title></circle>\n'

    # X labels
    for i in range(n):
        x = left_margin + (i / max(n - 1, 1)) * plot_width if n > 1 else left_margin + plot_width / 2
        ly = top_margin + plot_height + 16
        svg += f'<text x="{x}" y="{ly}" text-anchor="middle" font-size="10" fill="#555">{historical_data[i]["execution"]}</text>\n'
        svg += f'<text x="{x}" y="{ly + 13}" text-anchor="middle" font-size="9" fill="#888">{historical_data[i]["date"]}</text>\n'
    ...
    for _, label, color in metrics:
        legend += f'<span class="legend-item"><span class="legend-box" style="background:{color}"></span>{label}</span>\n'
    legend += '</div>\n'
...
def _render_historical_table(historical_data):
    html = '<table class="comp-table">\n<thead><tr>\n'
    headers = ["Execution", "Date", "Test Type", "Test Name", "Files",
               "Avg Total", "Avg WJ1", "Avg WJ2", "Avg WJ3", "Avg TPC (sec)"]
    for h in headers:
        html += f'<th class="run-header">{h}</th>\n'
    html += '</tr></thead>\n<tbody>\n'
    for d in historical_data:
        html += '<tr class="stage-row">'
        html += f'<td class="label-cell">{d["execution"]}</td>'
        html += f'<td class="time-val">{d["date"]}</td>'
        html += f'<td>{d["test_type"]}</td>'
        html += f'<td>{d["test_name"]}</td>'
        html += f'<td class="time-val">{d["file_count"]}</td>'
        html += f'<td class="time-val">{_secs_to_hms(d["avg_total"])}</td>'
        html += f'<td class="time-val">{_secs_to_hms(d["avg_wj1"])}</td>'
        html += f'<td class="time-val">{_secs_to_hms(d["avg_wj2"])}</td>'
        html += f'<td class="time-val">{_secs_to_hms(d["avg_wj3"])}</td>'
        html += f'<td class="time-val">{d["avg_tpc"]:.3f}</td>'
        html += '</tr>\n'
    html += '</tbody>\n</table>\n'
    return html


def generate_html_report(all_runs, output_path, historical_data=None):
    grouped = _group_runs_by_case_count(all_runs)
    insights = _generate_insights(grouped, all_runs, historical_data)

    has_history = historical_data and len(historical_data) > 0
    main_tabs = ["Key Insights", "Performance Results", "Stage Comparison Charts"]
    if has_history:
        main_tabs.append("Historical Trend (2K Bulk)")
    main_tabs.append("Raw Data")
    ...
    # TAB: Key Insights
    html += f'    <div class="main-tab-content active" id="main-tab-{tab_idx}">\n    <div class="card">\n'
    html += '        <h2>Key Insights &amp; Observations</h2>\n        <div class="insight-list">\n'
    for idx, pt in enumerate(insights, 1):
        color = pt["color"]
        bg = {"#C00000": "#FFF0F0", "#ED7D31": "#FFF8EE", "#006100": "#F0FFF0"}.get(color, "#F0F4FA")
    ...
    html += f'    <div class="main-tab-content" id="main-tab-{tab_idx}">\n    <div class="card">\n'
    ...
    # TAB: Stage Comparison Charts
    html += f'    <div class="main-tab-content" id="main-tab-{tab_idx}">\n'
    for cc, runs in grouped.items():
        html += f'    <div class="card">\n        <h2>Web Job Processing Time - {cc} Bulk Cases ({len(runs)} runs)</h2>\n'
        html += f'        <div class="chart-wrapper">\n{_render_bar_chart_svg(runs, cc, f"chart-{cc}")}\n        </div>\n    </div>\n'
    html += '    </div>\n'
    tab_idx += 1

    # TAB: Historical Trend (only for 2K bulk)
    if has_history:
        html += f'    <div class="main-tab-content" id="main-tab-{tab_idx}">\n'
        html += '    <div class="card">\n        <h2>2000 Bulk Case Creation - Historical Trend</h2>\n'
        html += '        <p style="color:#666;font-size:13px;margin-bottom:16px;">Trend data from GDC_RUN_LOG.xlsx showing average processing times per 2K file across executions.</p>\n'

        # Historical table
        html += '        <h3>Execution Summary</h3>\n        <div style="overflow-x:auto;">\n'
        html += _render_historical_table(historical_data)
        html += '        </div>\n'

        # Line chart: Avg Total Processing Time
        html += '        <h3>Average Total Processing Time Trend</h3>\n        <div class="chart-wrapper">\n'
        html += _render_trend_line_svg(historical_data, "avg_total", "Avg Total Processing Time (per 2K file)", "trend-total", LINE_COLORS["avg_total"])
        html += '        </div>\n'

        # Multi-line chart: WJ1, WJ2, WJ3
        html += '        <h3>Web Job Processing Time Trend</h3>\n        <div class="chart-wrapper">\n'
        html += _render_multi_trend_svg(historical_data, "trend-wj")
        html += '        </div>\n'
    ...
    html += '        <p style="color:#666;margin-bottom:16px;font-size:13px;">Complete job step data for all runs.</p>\n'
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .run-header { background: #4472C4; color: #fff; font-size: 12px; text-align: center; min-width: 140px; }
    ...
    .main-tab-nav { display: flex; gap: 0; margin-bottom: 0; border-bottom: 2px solid #4472C4; flex-wrap: wrap; }
    .main-tab-btn { padding: 12px 24px; background: #e8e8e8; border: 1px solid #ccc; border-bottom: none;
